...
const Index = () => {
    ...
    const getData = async () => {
        const res: any = await Models.lead.list(1);
        console.log('res: ', res);
        setState({ tableData: res.results });
        try {
            let res;
            ...
            <div className="panel mt-2 flex flex-col items-center justify-between gap-5 lg:flex-row">
                ...
                <div className="flex w-full flex-col gap-4 lg:w-2/3 lg:flex-row">
                    {ROLE.ADMIN == state.role && (
                        <>
                            <CustomSelect options={state.marketList} value={state.bdm} onChange={(e) => setState({ bdm: e })} isMulti={false} placeholder="BDM" />
                            <CustomSelect options={state.marketList} value={state.bde} onChange={(e) => setState({ bde: e })} isMulti={false} placeholder="BDE" />
                        </>
                    )}
                    {ROLE.BDM == state.role && <CustomSelect options={state.marketList} value={state.bde} onChange={(e) => setState({ bde: e })} isMulti={false} placeholder="BDE" />}

                    <CustomSelect options={state.marketList} value={state.market} onChange={(e) => setState({ market: e })} isMulti={false} placeholder="Status" />
                    <CustomeDatePicker
            ...
            <div>
                <DataTable
                    ...
                    columns={[
                        ...
                        { accessor: 'opportunity', title: 'Source', sortable: true },
